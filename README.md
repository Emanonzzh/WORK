...
| `agent_lab/tests/` | 245 项 pytest 单测（不连库、不调模型，0.3 秒跑完） | ⭐ 质量证据 |
...
- **条款依据可追**：切片时给每段打上 `articles` 元数据，检索工具把命中条款拼成 `【来源：第X条】`**喂给模型**，模型在回答里以自然语言引用具体条款。
  ⚠️ 说清楚边界：那个原始标签**不在界面上展示**，用户看到的是模型转述的条款号，不是可逐条点开的原文。
  要做到"用户自己核对原文"，还需把检索到的原文也渲染出来（**尚未做**）。
- **fail-fast 配置管理**：api_key 启动即校验；所有魔法数字收拢 `config.py` 并注明评测依据

## 演示截图

三张都从公网实例 `http://47.76.101.97` 实拍（2026-09-24），不是设计稿。
⚠️ 地址必须写全 `http://`——该入口没有 TLS，手机浏览器自动升级成 https 会打不开。

**① 首页：一个 Agent、三个工具**

![首页](screenshots/p2_01_home.png)

**② 形变风险分析：结构化报告 + 定量判据**

![形变风险分析](screenshots/p2_02_risk_report.png)

分数由**确定性规则**算出，不交给模型（阈值全部在 `config.py`）：
近 3 期平均月形变量 14.3 mm > 10 mm/月阈值 → **+40**；前半段 2.5 → 后半段 9.3 mm/期，判为加速 → **+30**；
最近一期 17.5 mm > 历史均值 5.7 × 3 = 17.1 mm，判为异常 → **+30**。合计 **100 分**，
按 `≤30 低 / ≤60 中 / >60 较高` 分级 → **较高风险**。

**③ 法规问答：结论落到具体条款**

![法规问答](screenshots/p2_03_regulation.png)

回答把依据写成"依据《地质灾害防治条例》第十二条…"，末尾固定挂免责声明：
*本系统结果仅基于公开数据辅助分析，不构成专业灾害预测或官方预警。*

> 截图受浏览器视口高度限制，② 的"结论/免责声明"与 ③ 之后的原文引用有一小段在画面外；
> 要看完整效果请直接开公网实例。
